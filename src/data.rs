...
//!
//! See [docs/data-api.md](../docs/data-api.md) for the full design. This module
//! is being built *alongside* the legacy `config`/`ctx` persistence layer; it is
//! not yet wired into `initialize_ctx`, so most of its surface is currently
...
const SCHEMAS: &[(&str, &str)] = &[
    ("id.yaml", include_str!("../resources/schemas/id.yaml")),
    ("units.yaml", include_str!("../resources/schemas/units.yaml")),
    ("settings.yaml", include_str!("../resources/schemas/settings.yaml")),
    ("stock.yaml", include_str!("../resources/schemas/stock.yaml")),
    ("cnc.yaml", include_str!("../resources/schemas/cnc.yaml")),
    ("fixture.yaml", include_str!("../resources/schemas/fixture.yaml")),
    ("toolset.yaml", include_str!("../resources/schemas/toolset.yaml")),
    ("machining.yaml", include_str!("../resources/schemas/machining.yaml")),
    ("catalog.yaml", include_str!("../resources/schemas/catalog.yaml")),
];
...
const CNC_TEMPLATES: &[(&str, &str)] = &[
    ("genmitsu_3018", include_str!("../resources/cnc_templates/genmitsu_3018.yaml")),
    ("masso_g3_with_atc", include_str!("../resources/cnc_templates/masso_g3_with_atc.yaml")),
    ("masso_g3_no_atc", include_str!("../resources/cnc_templates/masso_g3_no_atc.yaml")),
    ("batam", include_str!("../resources/cnc_templates/batam.yaml")),
];
...
impl AppData {
    ...
    pub fn load_from(data_dir: &Path, catalogs_dir: &Path) -> (Self, Vec<DataError>) {
        ...
        let settings_text = fs::read_to_string(&settings_path).ok();
        // A stock.yaml written by the retired legacy `save_stock` lacks the
        // `schema_version` field that stock.yaml's `x-schema-version` gating now
        // requires; inject it on load so such a file still parses (and can then be
        // rewritten in modern form by the AppData writer).
        let stock_text = fs::read_to_string(&stock_path).ok().map(|text| normalize_stock_text(&text));
...
// changes via a separate render-counter signal (see `ui::data_bind`).
// ---------------------------------------------------------------------------
...
pub fn init_appdata() -> Vec<DataError> {
    match crate::user_path::ensure_app_dirs() {
        Ok(dirs) => {
...
/// Normalizes on-disk stock text into a form current `stock.yaml` accepts: injects
/// `schema_version: 1` when absent (a legacy `save_stock` file omits it, which the
/// datastore's version gating would otherwise reject). Returns a JSON string (a
/// superset-compatible input for the YAML-or-JSON parser); on any parse failure it
/// passes the original text through unchanged so the normal error path reports it.
fn normalize_stock_text(text: &str) -> String {
    let Some(mut value) = parse_yaml_value(text) else {
...
mod tests {
    ...
    fn stock_replace_from_value_persists_and_reloads() {
        ...
        use crate::domain::stock::{stock_value_from_tools, Tool, ToolPreference, ToolStatus};
