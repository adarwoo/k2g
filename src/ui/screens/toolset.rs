...
use crate::ui::bindings::{
    clone_named, create_named_from_template, export_yaml, import_yaml, remove_profile_result, use_profiles, use_templates,
            ...
            div { class: "stock-toolbar",
                ...
                ProfileLifecycleToolbar {
                    ...
                    on_delete: move |_| {
                        ...
                        spawn(async move {
                            if super::profiles_common::confirm("Delete toolset profile", "Delete this toolset profile?").await {
                    ...
                    on_import: move |_| {
                        spawn(async move {
                            let Some(path) =
                                super::profiles_common::pick_import_file("Import toolset profile", "Toolset profile YAML")
                                    .await
                            else {
                                return;
                            };
                ...
                ProfileNameDialog {
                    ...
                    on_submit: move |_| {
                        ...
                        let result = if is_clone {
                            ...
                            create_named_from_template(Profile::Toolset, &selected_template.read(), &name)
                        };
                        match result {
                            Some(id) => {
                                selected.set(Some(id));
