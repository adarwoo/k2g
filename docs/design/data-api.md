...
3. **No duplicate persistence.** `datastore`'s background, coalescing, atomic
   writer replaces `legacy_config/persistence.rs` and
   `legacy_config/manager.rs`. Edits *are* saves.
...
- Code identifiers: `process_profile`, `processing`, `JobProfile` naming, the
  `resolve_schema_path` map in manager.rs:487,
...
- legacy_config/manager.rs `YamlConfigManager` — replaced by
  `datastore` parse/validate/default.
- legacy_config/persistence.rs `PersistenceWriteManager`,
